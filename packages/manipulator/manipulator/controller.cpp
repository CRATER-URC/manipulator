...
//       "~/joint_trajectory", rclcpp::SystemDefaultsQoS(), callback);
  ...
  auto callback =
    [this](const std::shared_ptr<sensor_msgs::msg::JointState> msg_) -> void
  {
    msg_external_point_ptr_.writeFromNonRT(msg_);
    new_msg_ = true;
  };
  ...
  const rclcpp::Time & time, const rclcpp::Duration & /*period*/)
{
  if (new_msg_)
  {
    msg_ = *msg_external_point_ptr_.readFromRT();
    start_time_ = time;
    new_msg_ = false;
  }
  ...
  if (msg_ != nullptr)
  {
    ...
    {
      joint_position_command_interface_[i].get().set_value(msg_.position[i]);
    }
    for (size_t i = 0; i < joint_velocity_command_interface_.size(); i++)
    {
      joint_velocity_command_interface_[i].get().set_value(msg_.velocity[i]);
    }
  }
