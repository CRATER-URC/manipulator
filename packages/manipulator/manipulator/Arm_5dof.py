...
class DemoNode(Node):
    ...
    def getq(self, pd, vd):
        ...
        # angle that the tip is rotated about the table
        thetaz = np.arctan2(R[1][0], R[0][0])

        # angles = [[thetaz], [thetax]]

        # self.get_logger().info(f"New  = {angles}")

        p = np.concatenate((p, angles))

        J = np.vstack((Jv, [0, 1, 1, -1, 0], [1, 0, 0, 0, -1]))

        Jinv = np.transpose(J) @ np.linalg.pinv((J @ np.transpose(J) + self.gamma**2 * np.eye(len(J))))

        # print("in demo, pd : ", pd, " p : ", p)
    ...
    def sendcmd(self):
        ...
        # when goal is detected from from Point node
            ...
            if self.t < self.startmov + self.T:
                ...
                self.qdot, self.q = self.getq(pd, vd)
            else:
                self.goalpos = None
                if not self.printed:
